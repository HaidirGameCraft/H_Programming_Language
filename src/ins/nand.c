...
SIGN_INSTRUCTION_SET(nand) {
    if( opcode == OPCODE_NAND_1 )
        instruction_nand_regmem_reg(prefix, opcode, memory, reg, pc);
    else if ( opcode == OPCODE_NAND_2 )
        instruction_nand_value_reg(prefix, opcode, memory, reg, pc);
}
...
uint32_t create_instruction_nand_regmem_reg(CREATE_INSTRUCTION_ARGS) {
    if( strncmp("nand", opcode, 4) != 0 )
        return pc;

    uint8_t prefix = PREFIX_RM_INC;
    uint8_t op = OPCODE_NAND_1;
