...
SIGN_INSTRUCTION_SET(sh) {
    if( opcode == OPCODE_SH_1 )
        instruction_sh_reg_reg(prefix, ext_prefix, opcode, memory, reg, pc );
    else if ( opcode == OPCODE_SH_2 )
        instruction_sh_reg_value( prefix, ext_prefix, opcode, memory, reg, pc );
}

void instruction_sh_reg_reg(INSTRUCTION_SET_ARGS) {
...
void instruction_sh_reg_value(INSTRUCTION_SET_ARGS) {
    VARIABLE_INSTRUCTION
    REGMEM_DEFINED(memory, pc);
