...
SIGN_INSTRUCTION_SET(nor) {
    if( opcode == OPCODE_NOR_1 )
        instruction_nor_regmem_reg(prefix, opcode, memory, reg, pc);
    else if ( opcode == OPCODE_NOR_2 )
        instruction_nor_value_reg(prefix, opcode, memory, reg, pc);
}
...
uint32_t create_instruction_nor_regmem_reg(CREATE_INSTRUCTION_ARGS) {
    if( strncmp("nor", opcode, 3) != 0 )
        return pc;

    uint8_t prefix = PREFIX_RM_INC;
    uint8_t op = OPCODE_NOR_1;
