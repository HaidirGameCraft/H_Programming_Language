...
SIGN_INSTRUCTION_SET(add) {
    if( opcode == OPCODE_ADD_1 )
        instruction_add_regmem_reg(prefix, opcode, memory, reg, pc );
    else if ( opcode == OPCODE_ADD_3 )
        instruction_add_value_reg(prefix, opcode, memory, reg, pc );
}
