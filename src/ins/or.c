...
SIGN_INSTRUCTION_SET(or) {
    if( opcode == OPCODE_OR_1 )
        instruction_or_regmem_reg(prefix, opcode, memory, reg, pc);
    else if ( opcode == OPCODE_OR_2 )
        instruction_or_value_reg(prefix, opcode, memory, reg, pc);
}
...
uint32_t create_instruction_or_regmem_reg(CREATE_INSTRUCTION_ARGS) {
    if( strncmp("or", opcode, 2) != 0 )
        return pc;

    uint8_t prefix = PREFIX_RM_INC;
    uint8_t op = OPCODE_OR_1;
