...
SIGN_INSTRUCTION_SET(xor) {
    if( opcode == OPCODE_XOR_1 )
        instruction_xor_reg_reg(prefix, ext_prefix, opcode, memory, reg, pc);
    else if ( opcode == OPCODE_XOR_2 )
        instruction_xor_value_reg(prefix, ext_prefix, opcode, memory, reg, pc);
}

void instruction_xor_reg_reg(INSTRUCTION_SET_ARGS) {
