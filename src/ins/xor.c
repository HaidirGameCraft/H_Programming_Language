...
SIGN_INSTRUCTION_SET(xor) {
    if( opcode == OPCODE_XOR_1 )
        instruction_xor_regmem_reg(prefix, opcode, memory, reg, pc);
    else if ( opcode == OPCODE_XOR_2 )
        instruction_xor_value_reg(prefix, opcode, memory, reg, pc);
}

void instruction_xor_regmem_reg(INSTRUCTION_SET_ARGS) {
...
void instruction_xor_value_reg(INSTRUCTION_SET_ARGS) {
    VARIABLE_INSTRUCTION
    memset( &__regmem, read8( memory, pc ), sizeof( __regmem ) );
    if( prefix & PREFIX_32BITS )
    {
...
uint32_t create_instruction_xor_regmem_reg(CREATE_INSTRUCTION_ARGS) {
    if( strncmp("xor", opcode, 3) != 0 )
        return pc;

    uint8_t prefix = PREFIX_RM_INC;
    uint8_t op = OPCODE_XOR_1;
...
uint32_t create_instruction_xor_value_reg(CREATE_INSTRUCTION_ARGS) {
    if( strncmp("xor", opcode, 3) != 0 )
        return pc;

    uint8_t prefix = PREFIX_RM_INC;
    uint8_t op = OPCODE_XOR_2;
    uint8_t dreg = regbytext(rReg);
