...
SIGN_INSTRUCTION_SET(mul) {
    if( opcode == OPCODE_MUL_1 )
        instruction_mul_regmem_reg( prefix, opcode, memory, reg, pc );
    else if ( opcode == OPCODE_MUL_2 )
        instruction_mul_value_reg( prefix, opcode, memory, reg, pc );
}
