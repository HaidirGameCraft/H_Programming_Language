...
void enable_raw_mode() {
    ...
    DWORD new_out_mode = out_mode;

    new_in_mode &= ~(ENABLE_ECHO_INPUT | ENABLE_LINE_INPUT);
    new_in_mode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;

    new_out_mode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;

    SetConsoleMode( hstdin, new_in_mode );
    SetConsoleMode( hstdout, new_out_mode );
#else
...
void display_register(int start_x, int width) {
    ...
    printf("\x1b[%iG RD: 0x%.08x\x1b[1B", start_x, reg.rf);
